...
local function get_oidc_config(http_client, endpoint)
  local res = http_client.get(resty_url.join(endpoint,'/.well-known/openid-configuration'))

  local config

  if res.status == 200 then
    config = cjson.decode(res.body)
  end

  ngx.log(ngx.DEBUG, '[oidc]: request: ', endpoint, ' response status: ', res.status, ' body: ', res.body)

  return config
end

local function get_public_key(http_client, self)
  local res = http_client.get(self.config.jwks_uri)

  local keys
  if res.status == 200 then
    keys = cjson.decode(res.body)
  end

  ngx.log(ngx.DEBUG, '[oidc]: request: ', self.config.jwks_uri, ' response status: ', res.status, ' body: ', res.body)

  return format_public_key(keys[0].n)
end

function _M.enabled()
  return env.get('OIDC_ENDPOINT')
end

function _M.load_configuration(client)
  local endpoint = env.get('OIDC_ENDPOINT')
  ...
  local oidc_config = get_oidc_config(http_client, endpoint)
...
function _M:authorize(service, client)
  ...
  local url = resty_url.join(self.config.authorization_endpoint, ngx.var.is_args, ngx.var.args)
...
function _M:get_token(service, client)
  ...
  local url = self.config.token_endpoint
